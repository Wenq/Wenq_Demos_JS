...
```
每一个新闻都是一个独立个体（实例对象），互不影响，避免相互耦合，也增强了组合后模块的复杂性。
...
//获取用户输入框中输入信息
var text = $('user_input'); //得到一个dom对象
//如果消息为空则提交失败
if(text.value === ''){
    return;
}
//发布一则评论消息
Observer.fire('addCommnetMessage', {
...
```
通过观察者/消息发布订阅模式，解决了不同模块（或不同类、对象）之间的耦合。
...
Student.prototype.answer = function(question){
    //注册参数问题
    Oberver.regist(question, this.say);
}
// 还有学生睡着了，无法回答问题
...
var student2 = new Student('学生2 回答问题');
var student3 = new Student('学生3 回答问题');
...
# 第18章 超级玛丽 - 状态模式（State）
### 2024/8/27
当一个对象内部状态发生改变时，会导致其行为的改变，看起来像改变了对象。

场景例子：评选最美图片，有很多图片，大家来打分，每个图片都有自己分数，且得分情况很多。

对于这种场景，我们可以将不同判断结果封装在状态对象内，然后该状态对象返回一个可被调用的接口方法，用于调用对象内部某种方法。
```javascript
//投票结果状态对象
...
```
上面代码为状态模式的基本雏形。
`状态模式`：主要目的就是将条件判断的不同结果转化为状态对象的内部状态。即为状态对象内部状态，所以一般作为状态对象内部私有变量，提供一个能够调用状态对象内部状态的接口方法对象。方便对状态对象中内部状态进行管理（在一个对象内部进行增加、修改、删除）。

`更复杂场景`：超级玛丽游戏，玛丽要吃蘑菇、要跳跃、要发子弹、要蹲下、要奔跑，同时这些动作还会组合，比如边奔跑变发子弹等。

通过if或者switch来处理这些状态变化，以及多个状态的组合时不靠谱，且难以维护的。特别是组合场景下，代码编写成本非常高。更别说以后新增、修改状态等。

`优化后的解决思路`：首先创建一个状态对象，内部保存状态变量，然后内部封装好每种动作对应的状态，最后状态对象返回一个接口对象，它可以i对内部的状态修改或调用。
### 2024/8/28
```javascript
//创建超级玛丽状态类
...
状态模式：可以对条件判断中的每一种情况独立管理，解决条件分支之间的耦合问题。
`最终目的`：简化分支判断流程。

# 第19章 活诸葛-策略模式
策略模式（Strategy）：将定义的一组算法封装起来，使其相互之间可以替换。封装的算法具有一定独立性，不会随客户端变化而变化。

策略模式与状态模式差异：策略模式不需要管理状态，状态间没有依赖关系，策略之间可以相互替换，在策略对象内部保存的是相互独立的一些算法。
...
controller控制器对象要操作view视图层对象与model模型层对象。
```javascript
//MVC控制器层
MVC.ctrl = function(){
    //模型数据层对象操作方法引用
...
view层不直接引用Model层内的数据，而是通过Presenter层实现对Model层内的数据访问。即所有层次的交互都发生在Presenter层中。

`解决的场景问题`：
MVC模式，view层因渲染会直接引用model层数据，但controller层无法感知。因此model的数据改变，controller常不知情但view层发生了变化。
即：controller不知道view在自己内部改了model什么东西。

### model数据模型层与view视图层耦合的代价（问题）

问题：会导致新增需求时如果需求较复杂，会改动很大，成本高
解决办法：将view视图层与model数据层解藕，将相关逻辑交给controller控制层处理；该模式是MVC的优化，即MVP模式。

MVP的P是presenter管理层，负责：管理数据、UI视图创建、交互逻辑和动效等一切事务。这样数据层只负责存储数据，视图层只负责创建视图模版。

MVC -》 MVP：（MVC模式演变为MVP模式）
数据层不需要太多变化，view视图层需要做大的更改。
...
从MVC演化到MVP，其view视图层需要做大的改动。

```javascript
//为了在P管理层中渲染并创建视图，渲染每一个导航都需要视图层提供一个导航视图模版
var tpl = [
    '<li>',
        '<a>',
            '<li />',
...
//视图层
MVP.view = MPV.view = (function(){ //这里为什么有两个MVP.viwe？？
    return function(str){
        //将参数字符串转换为期望的模版
        return html;
    }
...
# 第四十章 MVVM模式
### 2024/4/15
MVVM模式，Model（模型） - View（视图） —— ViewModel（视图模型）。
视图模型viewmodel：
1、为视图层view量身定做的一层、
2、在视图模型viewmodel中可以创建属性和方法、
3、为视图层view绑定数据model，并实现交互。

`使用场景`：某些情况下，一些开发者对于复杂的javascript了解得不是很深入，操作MVP模式中p即管理器成本很大，**希望直接通过html创建视图实现页面需求**。

## MVC、MVP和MVVM设计模式的来源
以下答案由GPT提供：
1、MVC模式由Trygve Reenskaug在1979年为Smalltalk语言设计时提出。
2、MVP模式是在MVC基础上自然演化的产物，没有特定的创始人。
3、MVVM模式则是由微软工程师John Gossman在2005年前后为WPF和Silverlight平台设计时明确提出的。

###  2024/4/17
MVVM模式中，VM是核心。

创建视图即是创建页面内的视图，因此本质上就是在页面中书写HTML代码，因此如果将视图作用提升，通过在页面中直接书写HTML代码创建视图组件，让控制器或管理器去监听这些视图组件，并处理这些组件完成预期功能，这种实现方式可以降低开发的门槛，让只懂HTML代码的开发者更轻松完成功能需求。  --这即是MVVM模式
...
```
`感受`：上面的html代码其实就是想实现“绑定”，视图与数据的绑定（单向或双向）。

上面代码是想通过data-bind自定义属性值为元素绑定javacript行为。
view视图层内的元素要被视图模型viewmodel层监听，所以会在vm层中实现对这些元素的监听，并绑定行为。
...
(function(){
    //在闭包中获取全局变量
    var window = this || (0, eval)('this'); //在全局上下文中获取window对象的引用（eval('this')是在全局作用域下执行，所以返回window对象）
    //获取页面字体大小，作为创建页面UI尺寸参数物
    var FONTSIZE = function(){
        //获取页面body元素字体大小并转化为整数（currentStyle属性用于兼容IE）
...
```
如果要添加其他UI组件创建方法，可在上面VM中拓展添加（目前支持2个：progressbar、slider）。
...
slider : function(dom, data){
    //
    var bar = doucument.createElement('span');
    //
...
### 让滑动条动起来
### 2024/4/21
为滚动条添加滚动交互事件，这样才能让用户拖动使用：
大概交互 - 用户拖动滚动条拨片，则滚动条文案和拨片位置以及进度条容器宽度都要随之改变。用户放开拖动，滑动交互停止。
```javascript
//滑动条交互
slider : function(dom, data){
    //创建组件逻辑
...
```
### 为组件点睛（实现数据监听与绑定）
1、创建组件的方法是VM对象中偏重于view视图的逻辑
2、完整的VM对象还需要对model模型对象的数据处理

目标：在VM中实现一个获取数据的方法getBindData，通过元素中给定的数据映射标识获取对应数据来渲染视图view。
...
var VM = function(){
    ...
    function getBindData(dom){
        //获取组件自定义属性data-bind值(数据示例: "type:'slider', data:demo1")
...
var VM = function(){
    ...
    return function(){
        ...
        //元素自定义数据句柄
        ctx = null;
        //ui处理实惠想页面中插入元素，此时doms.length会改变，此时动态获取dom.length
...
```
`MVVM模式`：是由MCV、MVP演变而来，因此其主要用途还是用来分离视图view和数据模型Model。
1、不同在于MVVM模式使视图层view更灵活，可独立于model数据层，VM视图模型层而独立修改，自由创建。
2、这也使得model数据模型层可独立变化，甚至一个视图模型层可以对应多个view视图层或vm数据模型层。
3、MVVM模式是对vm视图模型层的高度抽象...
4、MVVM模式最重要特征是view视图层独立开发，使得不太懂js的人只要了解HTML内容并按照视图层规范格式创建视图即可完成一个复杂页面开发。同时让开发人员可专注于开发vm视图模型层里的业务逻辑。
5、测试也变得容易，只需要针对vm视图模型层写测试代码即可。
